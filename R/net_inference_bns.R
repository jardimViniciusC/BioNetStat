#' 'adjacencyMatrix' creates a function that infers a graph from gene expression 
#' data.
#' @param method: a function that measures the association between the variables values.
#' @param association: a charactere string indicating wich value will be used as association value. 
#' The options are "corr" for the correlation value, "pvalue" for nominal pvalue associated to 
#' correlation or "fdr" for corrected pvalue for mutiple tests.
#' @param threshold: a charactere string indicating wich value will be used as threshold value. 
#' The options are "corr" for the correlation value, "pvalue" for nominal pvalue associated to 
#' correlation or "fdr" for corrected pvalue for mutiple tests. If NULL, no edge is removed.
...
adjacencyMatrix <- function(method, association=c("corr","pvalue","fdr"), threshold=c("fdr", "pvalue","corr", NULL),
                            thr.value=0.05, weighted=T) {# Lembrar que o threshold pode ser para a correlacao ou para p-valor
  return(
    function(expr) {
      ...
      if(any(method==c("pearson","spearman"))){
        if(nrow(expr)==3 | nrow(expr)==4) A <- list(cor(as.matrix(expr), method=method))
          else A <- Hmisc::rcorr(as.matrix(expr), type=method) #tem que estar como matriz, não le data frames.
      }
      if(method=="kendall"){
        if(nrow(expr)==3 | nrow(expr)==4) A <- list(cor(as.matrix(expr), method=method))
          else {
